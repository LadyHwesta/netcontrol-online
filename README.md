...
Every registered operator has a base org-management tier — **Admin** (approving members, branding, etc.) or plain member — separate from four participant roles they can hold in any combination: **Net Control Op** (everyday net access; this is what "Member" used to be called, with identical privileges), **Tactical Operator**, **Broadcaster**, and **Fediverse Operator**. All four are independently toggleable, for anyone, from the Operators list in Admin — click a role badge to grant or revoke it, the same for all four (an Admin can hold any of them too, e.g. a founder who also runs nets). The registration form lets a new operator flag which they're interested in as a hint; the org admin decides what's actually granted, defaulting Net Control Op to checked (the normal case) on the pending-approval queue.

Holding a role at the org level only makes it *offerable* — it still has to be granted per net via that net's **🔗 Sharing** section (Edit Net → Sharing), the same place edit rights (`Can edit`, i.e. Net Control Op access) are already granted. A user can hold different roles on different nets: Net Control Op on one, Tactical Operator on another, both on a third. Tactical Operator/Broadcaster sharing only ever offers a role the org admin already approved for that user — greyed out otherwise, right in the sharing picker — so revoking it at the org level takes it away everywhere at once. `Can edit` (Net Control Op's net-level grant) and **Fediverse Operator** are the exceptions and stay ungated by any net's Sharing section — both are org-wide (one Fediverse actor per org, not per net), so holding the role grants access directly (see [Interaction client](#interaction-client)).
...
- **Net Control rotation shift changes during an activation** — push-only, since there's no signup email address for this occasion. Whoever's up next in the rotation queue gets a push shortly before their shift starts, matched to their account by callsign (a queued shift is always free-text callsign/name, same as the rotation itself already is — if the callsign doesn't match a registered account, there's simply no one to push to, same as today).
...
A small built-in client (the **🐘 Fediverse** sidebar page) closes the loop on the one-way posting above: it shows replies and Likes your org's posts receive, lets you reply and Like back, and lets you compose an ad-hoc post outside the automatic start/end announcements. Available to an org admin or anyone holding the **Fediverse Operator** role (requestable at registration/join, or granted under Admin → Operators, same as Tactical Operator/Broadcaster) — turning the account on/off and setting hashtags stays admin-only. Out of scope by design: browsing or searching the wider Fediverse, and boosting (Announce) — this only covers interactions on the org's own posts.

A remote reply's content is untrusted third-party HTML and is stripped down to plain text server-side before it's ever stored or shown, so a hostile reply can't inject markup/scripts into the client.
