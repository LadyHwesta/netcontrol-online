...
- **County, full static catalog** — currently **Sonoma County, CA**'s own "Know Your Zone" GIS system, also a public ArcGIS FeatureServer. Set the net's **Region** field to `Sonoma` (or `Sonoma County`). Unlike the statewide feed, this always contains every predefined zone in the county — hundreds of them, each showing `Normal` status outside an incident — so it's what makes zone selection actually useful on a routine net, not just during an activation.
