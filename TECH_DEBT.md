...
### UI translation (argos-translate) covers only the auth screen, shared nav, and Nets view so far
The translation framework itself (`routers/translation.py`, `static/js/i18n.js`, the admin Languages tab, `extract_i18n_strings.py`) is complete and works end-to-end. Coverage is being extended incrementally, one real-estate slice at a time: the auth/login screen and shared sidebar nav (2026-08-31), then the Nets view — the New/Edit Net form and its toasts (2026-08-31, `nets.js`/`index.html`'s `#view-nets`). Everything else is still plain hardcoded English:
...
- **Everything else**: `sessions.js`, `schedules.js`, `history.js`, `dmr.js`, `aprs.js`, and their corresponding sections of `index.html` (`#view-session` and beyond, including the live check-in panel), plus all of `admin.html`, `help.html`, `tokens.html`, `report.html` beyond the shared nav.
